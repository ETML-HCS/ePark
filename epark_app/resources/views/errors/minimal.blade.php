...
<head>
    ...
    <title>Erreur {{ $__exception->getStatusCode() }} - ePark</title>
    ...
    <div class="max-w-2xl w-full text-center">
        ...
        <div class="mb-8">
            <div class="inline-flex items-center justify-center w-32 h-32 bg-gradient-to-br from-gray-400 to-gray-600 rounded-full shadow-2xl mb-4">
                <span class="text-5xl font-black text-white">{{ $__exception->getStatusCode() }}</span>
            </div>
        </div>

        <!-- Message principal -->
        <h1 class="text-5xl font-black text-gray-900 mb-4">
            @switch($__exception->getStatusCode())
                @case(403)
                    Accès refusé
                    @break
                @case(419)
        ...
        <p class="text-lg text-gray-600 mb-8">
            @switch($__exception->getStatusCode())
                ...
                @case(503)
                    ...
                    @break
                @default
                    {{ $__exception->getMessage() ?: 'Une erreur inattendue s\'est produite.' }}
            @endswitch
        </p>

        @if(app()->environment('local') && isset($__exception))
            <div class="mb-8 p-6 bg-yellow-50 border-2 border-yellow-200 rounded-2xl text-left">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm font-bold text-yellow-800 mb-1">{{ get_class($__exception) }}</p>
                        <p class="text-sm text-yellow-700 font-medium">{{ $__exception->getMessage() }}</p>
                        @if($__exception->getFile())
                            <div class="text-xs text-yellow-600 font-mono bg-white rounded-lg p-3 mt-3">
                                <strong>Fichier :</strong> {{ $__exception->getFile() }}<br>
                                <strong>Ligne :</strong> {{ $__exception->getLine() }}
                            </div>
                        @endif
                    </div>
                </div>
