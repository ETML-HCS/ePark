        ...
        <div class="flex items-center justify-between">
            ...
            @if($unreadTotal > 0)
                <form method="POST" action="{{ route('notifications.markAllRead') }}">
                    @csrf
                    <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-xl text-sm font-bold text-gray-700 hover:bg-gray-50 transition-all shadow-sm">
    ...
    <div class="py-8 bg-gradient-to-b from-gray-50 to-white min-h-screen">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            @if($notifications->isEmpty())
                <div class="bg-white shadow-sm rounded-2xl border border-gray-100 p-12 text-center">
                    <div class="flex flex-col items-center">
                        ...
                        <p class="text-lg font-semibold text-gray-900 mb-1">Aucune notification</p>
                        <p class="text-sm text-gray-500">Vous serez notifié lors de nouvelles réservations ou rappels.</p>
                    </div>
                </div>
            @else
                <div class="space-y-3">
                    @foreach($notifications as $notification)
                        ...
                        <a href="{{ url('/reservations/' . ($payload['reservation_id'] ?? '')) }}"
                           class="group block bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden {{ $isUnread ? 'border-indigo-200 ring-1 ring-indigo-100' : 'border-gray-100' }}">
                            <div class="flex items-start gap-4 p-5">
                                {{-- Icône contextuelle --}}
                                <div class="flex-shrink-0 p-2.5 rounded-xl {{ $isEndReminder ? 'bg-amber-100' : ($isNewReservation ? 'bg-green-100' : 'bg-indigo-100') }}">
                                    @if($isEndReminder)
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    @if($placeName)
                                        <p class="text-sm text-gray-600 mt-0.5 truncate">{{ $placeName }}</p>
                                    @endif
