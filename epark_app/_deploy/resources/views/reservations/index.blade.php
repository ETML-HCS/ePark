    ...
    <div class="py-8 bg-gradient-to-b from-gray-50 to-white min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 bg-white rounded-2xl shadow-sm border border-gray-100 p-5" x-data="{ showFilters: false }">
                ...
                <form method="GET" action="{{ route('reservations.index') }}" x-show="showFilters" x-transition class="mt-4 pt-4 border-t border-gray-100">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div>
                            <label for="filter-period" class="block text-sm font-bold text-gray-700 mb-2">Période</label>
                            <select id="filter-period" name="period" class="w-full rounded-xl border-2 border-gray-200 bg-gray-50 focus:ring-0 focus:border-indigo-500 transition-all">
                                <option value="upcoming" {{ request('period', 'upcoming') === 'upcoming' ? 'selected' : '' }}>À venir</option>
                                <option value="past" {{ request('period') === 'past' ? 'selected' : '' }}>Passées</option>
                                <option value="all" {{ request('period') === 'all' ? 'selected' : '' }}>Toutes</option>
                            </select>
                        </div>
                        <div>
                            <label for="filter-status" class="block text-sm font-bold text-gray-700 mb-2">Statut de réservation</label>
                            <select id="filter-status" name="status" class="w-full rounded-xl border-2 border-gray-200 bg-gray-50 focus:ring-0 focus:border-indigo-500 transition-all">
                        ...
                        <div>
                            <label for="filter-payment" class="block text-sm font-bold text-gray-700 mb-2">Statut de paiement</label>
                            <select id="filter-payment" name="payment" class="w-full rounded-xl border-2 border-gray-200 bg-gray-50 focus:ring-0 focus:border-indigo-500 transition-all">
                                <option value="" {{ request('payment') === null || request('payment') === '' ? 'selected' : '' }}>Tous les paiements</option>
                                ...
                                <option value="failed" {{ request('payment') === 'failed' ? 'selected' : '' }}>Échec</option>
                                <option value="refunded" {{ request('payment') === 'refunded' ? 'selected' : '' }}>Remboursé</option>
                            </select>
                        </div>
                    </div>
                    <div class="flex items-center justify-end gap-3">
                        <button type="submit" class="px-4 sm:px-6 py-2.5 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all shadow-sm hover:shadow-md flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            </svg>
                            <span class="hidden sm:inline">Appliquer les filtres</span>
                        </button>
                        <a href="{{ route('reservations.index') }}" class="px-4 sm:px-6 py-2.5 bg-gray-100 text-gray-700 rounded-xl font-bold hover:bg-gray-200 transition-all flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
